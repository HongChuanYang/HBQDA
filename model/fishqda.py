...
class SentAttNet_fishqa(nn.Module):
    ...
    def forward(self, input, query_emb):
        ...
        # alpha_q1 = torch.matmul(self.att_net_1(f_output), query_emb[0]) / np.sqrt(d_K)
        # alpha_q2 = torch.matmul(self.att_net_2(f_output), query_emb[1]) / np.sqrt(d_K)
        # alpha_q3 = torch.matmul(self.att_net_3(f_output), query_emb[2]) / np.sqrt(d_K)
        # alpha_q4 = torch.matmul(self.att_net_4(f_output), query_emb[3]) / np.sqrt(d_K)
        # alpha_q5 = torch.matmul(self.att_net_5(f_output), query_emb[4]) / np.sqrt(d_K)
        ...
        # alpha_mean = (alpha_t+alpha_q1+alpha_q2+alpha_q3+alpha_q4+alpha_q5)/6
        # alpha_s = F.softmax(alpha_mean, dim=1)
        ...
        # att_a = torch.softmax(torch.matmul(alpha_v.unsqueeze(1), alpha_q_all.permute(0,2,1)) * 10000, -1)
        att_a = torch.softmax(torch.matmul(alpha_v.unsqueeze(1), alpha_q_all.permute(0,2,1)) / np.sqrt(d_a) , -1)
        # alpha_vqs = torch.softmax(alpha_vq, -1)
