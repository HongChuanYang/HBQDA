...
class Fishqa_trainer(object):
    ...
    def train(self, config, model, train_iter, dev_iter, test_iter, query_ids):
        ...
        for epoch in range(self.num_epochs):
            ...
            for i, (trains, labels, index) in loop:
                ...
                optimizer.zero_grad()
                with autocast():
                    outputs, all_q_value= model(trains, query_ids)
                ...
                if total_batch % interval_num == 0:
                    ...
                    # loop.set_postfix({'Iter':'{0:>6}'.format(total_batch), 'Train Loss':'{1:>5.2}'.format(loss_avg())})
                    # loop.set_postfix(Iter = total_batch, TrainLoss = loss_avg(), Trainf1=train_f1, ValLoss=dev_loss)
                    loop.set_postfix_str(msg.format(total_batch, loss_avg(), train_f1, dev_loss, dev_f1, dev_acc, time_dif, improve))
                    # loop.set_postfix(msg.format(total_batch, loss_avg(), train_f1, dev_loss, dev_f1, dev_acc, time_dif, improve))

                    # print(msg.format(total_batch, loss_avg(), train_f1, dev_loss, dev_f1, dev_acc, time_dif, improve))
                    # model.train()
                total_batch += 1
    ...
    def test(self, config, model, test_iter, query_ids, start_time):
        ...
        # start_time = time.time()
        test_f1, test_acc, test_loss, test_report, test_confusion, labels_all, q_value_all, predict_all, index_all = self.evaluate(config, model, test_iter, query_ids, test=True)
        ...
        pd_labels_all = pd.DataFrame(labels_all)
        pd_labels_all['index'] = index_all
        pd_labels_all.to_csv(config.label_path, index=None)
        pd.DataFrame(predict_all).to_csv(config.predict_path, index=None)
        pd.DataFrame(q_value_all).to_csv(config.query_weight_path, index=None)
    ...
    def evaluate(self, config, model, data_iter, query_ids, test=False):
        ...
        with torch.no_grad():
            ...
            for i, (texts, labels, index) in enumerate(data_iter):
                with autocast():
                    outputs, all_q_value = model(texts, query_ids)
                ...
                all_q_value_list = all_q_value.cpu().numpy().tolist()
                index_l = index.data.cpu().numpy().tolist()
                for i_v in index_l:
                    index_all.append(i_v) 
                for q_v in all_q_value_list:
                    q_value_all.append(q_v)
        ...
        if test:
            ...
            return f1_score, acc, loss_avg(), report, confusion, labels_all.tolist(), q_value_all, predict_all.tolist(), index_all
        model.train()
